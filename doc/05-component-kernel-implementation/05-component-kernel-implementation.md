...
I chose a Map as the underlying data structure, where each key is an item name and each value stores the item's status and aisle location. Choosing Map I think was the best choice because all kernel methods look up items by name, it prevents duplicate entries, and it keeps status and location together in
one place.
